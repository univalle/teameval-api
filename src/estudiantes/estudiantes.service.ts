...
export class EstudiantesService {
    ...
    registrarEstudiante(obj_Estudiante:registrarEstudianteDto){
        ...
        this.lista_estudiantes.push({ //esto debiera reemplazarse por el push a la base de datos
    ...
    actualizarEstudiante(obj_Estudiante:actualizarEstudianteDto) {
        ...
        const indice = this.lista_estudiantes.findIndex(est => est.id === obj_Estudiante.id);
        
        // Si se encuentra el estudiante, actualiza la información
        if (indice !== -1) {
            this.lista_estudiantes[indice] = {...this.lista_estudiantes[indice], ...obj_Estudiante};
    ...
    eliminarEstudiante(idEstudiante:number) { //esta es una forma de eliminar aunque se puede ver otra
        // Encuentra el índice del estudiante en la lista que tiene la misma id
        const indice = this.lista_estudiantes.findIndex(est => est.id === idEstudiante);
